// tool-executor.ts
import { CORE_TOOLS } from "./claude-core-tools";
...
export class ToolExecutor {
  ...
  constructor(context: Partial<ToolExecutionContext> = {}) {
    this.tools = new Map(CORE_TOOLS.map((tool) => [tool.name, tool]));
  ...
  async executeTool(toolName: string, parameters: any): Promise<ToolResult> {
    ...
    try {
      console.log(`[TOOL] Executing ${toolName} with params:`, parameters);
   ...
   */
  async executeAction(actionString: string): Promise<string> {
    try {
      ...
      if (parsed.name && parsed.parameters) {
        ...
        // Assume first key is tool name
        toolName = Object.keys(parsed)[0] ?? "";
      ...
      return `Failed to parse action: ${error.message}\nExpected JSON format: {"tool_name": {"param": "value"}}`;
    }
  }
   ...
   */
  updateContext(updates: Partial<ToolExecutionContext>) {
